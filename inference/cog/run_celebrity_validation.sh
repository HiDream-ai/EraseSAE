...
GPU="${GPU:-0}"
CHECKPOINT_DIR="${CHECKPOINT_DIR:-checkpoints/cog/celebrity}"
PROMPTS_FILE="${PROMPTS_FILE:-data/inference_demo/cogvideo/celebrity.csv}"
TARGET_IDENTITIES="${TARGET_IDENTITIES:-trump,obama,musk,merkel,elizabeth,swift}"
VALIDATION_MODE="${VALIDATION_MODE:-self}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%d_%H%M%S)}"

# Move the target-identity region toward a shared non-identity reference.
REFERENCE_PROMPT="${REFERENCE_PROMPT:-nudity}"
MASK_CFG_SCALE="${MASK_CFG_SCALE:-3.0}"
MASK_MODE="${MASK_MODE:-intersection}"
MASK_BACKGROUND_THRESHOLD="${MASK_BACKGROUND_THRESHOLD:-0.1}"
MASK_CONCEPT_THRESHOLD="${MASK_CONCEPT_THRESHOLD:-0.4}"
...
COMPETITION_RATIO="${COMPETITION_RATIO:-0.95}"
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/inference/cog/celebrity_${VALIDATION_MODE}/toward_${CONCEPT_GATE_MODE}_${CONCEPT_GATE_POLICY}_r${COMPETITION_RATIO}_${RUN_ID}}"

...
from training.cog_online_data import create_online_cog_loader
...
class UnifiedCategoricalDataset(Dataset):
    def __init__(self, root_dir, layer_name, target_celebs=None, mean=None, std=None, device="cuda"):
        ...
        # 1. 扫描目录结构 (新结构适配)
        # -------------------------------------------------------
        celeb_root = self.root_dir / "celebrity"
        non_celeb_root = self.root_dir / "no_celebrity"

        if not celeb_root.exists():
            raise FileNotFoundError(f"Directory not found: {celeb_root}")

        # 获取所有名人文件夹
        all_celebs = sorted([d.name for d in celeb_root.iterdir() if d.is_dir()])

        # 🚀 核心修改：筛选名人
        if target_celebs is not None and len(target_celebs) > 0:
            # 过滤掉不在 target_celebs 中的文件夹
            self.celebrities = [c for c in target_celebs]
            print(f"🎯 [Filter Applied] Training on specific celebrities: {self.celebrities}")
            
        else:
            self.celebrities = all_celebs
            print(f"🌍 [All Included] Training on all found celebrities: {len(self.celebrities)}")

        # 建立名人的 ID 映射 (0, 1, 2...)
        self.celebrity_to_id = {name: i for i, name in enumerate(self.celebrities)}
        
        # 定义 non_celebrity 的特殊 ID
        #self.NON_CELEB_ID = -1 
        ...
        # -------------------------------------------------------
        self.all_samples = []
        def collect_from_dir(directory, is_celebs_folder):
            ...
            for sub in sub_dirs:
                ...
                if folder_name in self.celebrity_to_id:
                    # 如果在目标列表中，赋予对应 ID (Specific 分支)
                    label = self.celebrity_to_id[folder_name]
        ...
        # 分别扫描两个根目录
        collect_from_dir(celeb_root, is_celebs_folder=True)
        collect_from_dir(non_celeb_root, is_celebs_folder=False)

        print(f"✅ Dataset Initialized:")
        print(f"   - Root: {self.root_dir}")
        print(f"   - Celebrities count: {len(self.celebrities)}")
...
def create_dataloader_for_layer(data_root_dir, mean, std, layer_name: str, batch_size: int, rank: int, world_size: int, target_celebs=None):
    ...
    return data_loader,dataset.celebrities
...
def run_global_stats_collection(root_dir, layer_name, data_dim):
    ...
    pattern_celeb = f"celebrity/*/activation/{layer_fn}/*.pt"
    # 2. 匹配 non_celebrity 下的所有: root/non_celebrity/*/activation/{layer}/*.pt
    pattern_nc = f"non_celebrity/*/activation/{layer_fn}/*.pt"
    ...
    save_dir = root_path / "ce_global_stats"
...
def parse_target_layers(layer_str: str):
    """
    🚀 核心：适配 CogVideoX，并挂载到 FF 层获取增量
    """
    if '-' in layer_str:
        parts = layer_str.split('-')
        start, end_step = int(parts[0]), parts[1].split(':')
        end = int(end_step[0]) + 1
        step = int(end_step[1]) if len(end_step) > 1 else 1
        layers = [f"transformer_blocks.{i}.ff.net.2" for i in range(start, end, step)]
    elif ',' in layer_str:
        indices = [int(x.strip()) for x in layer_str.split(',')]
        layers = [f"transformer_blocks.{i}.ff.net.2" for i in indices]
    else:
        layers = [f"transformer_blocks.{layer_str}.ff.net.2"]
    return layers
...
def train_worker(rank, world_size, args):
    ...
    for layer_name in target_layers:
        ...
        if getattr(args, "activation_source", "offline") == "online":
            layer_dataloader = create_online_cog_loader(
                args=args,
                layer_name=layer_name,
                task="celebrity",
                device=device,
            ...
            # 统计量路径现在在 root/global_layer_stats/ 下
            stats_root = Path(args.data_save_dir) / "ce_global_stats"
            stats_path = stats_root / f"stats_{layer_fn}.pt"

            if rank == 0:
                stats_root.mkdir(exist_ok=True, parents=True)
                if not stats_path.exists():
                    # 🚀 这里的 calculate_global_stats 会扫描所有 Subject 目录
            ...
            # 🚀 DDP 同步点：确保 Rank 0 计算/加载完后，其他进程再读取
            dist.barrier()

            # 非 Rank 0 的进程从文件中加载统计量
            if rank != 0:
                stats_data = torch.load(stats_path, map_location="cpu")
                global_mean = stats_data['mean']
                global_std = stats_data['std']

            # 4. 创建 DataLoader (现在是 per-GPU 数据加载)
        ...
        dist.barrier()
        if rank == 0:
            print(f"\n🚀 [Phase 2] Starting Distributed Feature Mining on {world_size} GPUs...")
        sae.eval()
...
def main(args):
    launch_training(args, train_worker)
if __name__ == "__main__":
    parser = build_training_parser(
        description="SAE-based feature erasure for CogVideoX-5B",
        config_name="cogvideo_celebrity",
    )
